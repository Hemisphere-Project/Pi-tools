...
**Inhibit it for deliberate maintenance**: `touch /run/usbfix.inhibit` before a node OTA or a USB test, remove it after — otherwise the maintenance looks like the fault it is meant to cure (2026-09-22: HPlayer2 is stopped for a flash, so it logs no HELLO, and the watchdog re-enumerated the port at 88 % of a transfer). The same reasoning is built in: trigger C is skipped whenever the host is not running. An inhibit is logged once per **transition** (`inhibited — …` / `inhibit cleared — …`), not on every tick, so a `touch` left behind after maintenance shows up in `journalctl -t usbfix` instead of reading exactly like a card that saw no storms — and a reboot with the file still in place logs it again on the first run.
